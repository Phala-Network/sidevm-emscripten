...
/// setjmp
pub fn __setjmp(ctx: &EmEnv, _env_addr: u32) -> Result<c_int> {
...
pub fn __longjmp(ctx: &EmEnv, _env_addr: u32, _val: c_int) -> Result<()> {
...
#[allow(unreachable_code)]
pub fn _longjmp(ctx: &EmEnv, _env_addr: i32, _val: c_int) -> Result<()> {
    debug!("emscripten::__longjmp (longmp)");
    abort_with_message(ctx, "missing function: _longjmp")
}
